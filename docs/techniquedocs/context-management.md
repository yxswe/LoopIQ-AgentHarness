...
message and all corresponding results must remain on the same side of the cut.
Malformed call/result history is not repaired by compaction.
...
The bridge summary must preserve the original instruction, early work, and the
facts required to understand the retained suffix. This permits compaction
during a long tool-using Run without retaining the entire Run or leaving raw
tool results detached from their call.
...
and keeps back-to-back compaction requests bounded.
