...
The runtime now implements the event-specific reducer semantics in
`src/core/event-bus.ts`, and `AgentRun` depends only on typed `emitHook()`.
Context, before-agent, provider request/payload, tool call/result, and compaction
reducers match the rules below.
...
```ts
interface ContextEvent extends HookEvent<
  "context",
...
interface ToolCallEvent extends HookEvent<
  "tool_call",
