...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @loopiq/ai <command> [provider]
...
Examples:
  npx @loopiq/ai login              # interactive provider selection
  npx @loopiq/ai login anthropic    # login to specific provider
  npx @loopiq/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @loopiq/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @loopiq/ai --help' for usage`);
	process.exit(1);
}
