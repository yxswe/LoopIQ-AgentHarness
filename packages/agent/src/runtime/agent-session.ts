...
import type { AgentHookEvent, AgentHookEventResultMap } from "../base/events.ts";
import type { AgentSessionConfig, AgentStreamOptions, QueueMode, ThinkingLevel } from "../base/options.ts";
import type { AgentResources, AgentTool, PromptTemplate, Skill } from "../base/resource.ts";
...
import type { ModelReference, PersistedSessionConfig } from "./persisted-session-config.ts";
...
> extends AgentSessionConfig<TSkill, TPromptTemplate, TTool> {
	env: ExecutionEnv;
	session: Session;
	engine: AgentEngine;
	persistConfig?: (config: PersistedSessionConfig, writer: SessionWriter) => Promise<void>;
...
> {
	...
	private readonly resources: AgentResources<TSkill, TPromptTemplate>;
	private readonly streamOptions: AgentStreamOptions;
	private readonly systemPrompt: AgentSessionConfig<TSkill, TPromptTemplate, TTool>["systemPrompt"];
	...
	async abort(runId: string): Promise<AbortResult> {
		...
		if (errors.length > 0) {
			...
			throw normalizeRuntimeError(cause, "hook");
		}
	...
	getRuntimeConfig(): PersistedSessionConfig {
		return {
	...
	private getResources(): AgentResources<TSkill, TPromptTemplate> {
		return {
	...
	private validateUniqueNames(names: string[], message: string): void {
		...
		if (duplicates.length > 0) {
			throw new AgentRuntimeError("invalid_argument", `${message}: ${duplicates.join(", ")}`);
		}
	}
	...
	private validateToolNames(toolNames: string[]): void {
		...
		if (missing.length > 0) throw new AgentRuntimeError("invalid_argument", `Unknown tool(s): ${missing.join(", ")}`);
	}
}
