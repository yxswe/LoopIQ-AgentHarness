...
export function createAgentRunController(): AgentRunController {
	...
	return {
		...
		openInferenceScope(): InferenceScope {
			if (disposed) throw new AgentRuntimeError("invalid_state", "AgentRun control is disposed");
			if (activeScope) throw new AgentRuntimeError("invalid_state", "An inference scope is already active");
