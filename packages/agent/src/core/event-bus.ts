...
type AgentEventHandler = (event: any, signal?: AbortSignal) => Promise<any> | any;

/**
 * Registration and dispatch of Session runtime events.
 *
...
export class AgentEventBus<TSkill extends Skill = Skill, TPromptTemplate extends PromptTemplate = PromptTemplate> {
	...
	private getHandlers(type: string): Set<AgentEventHandler> | undefined {
	...
	subscribe(
		...
		handlers.add(listener as AgentEventHandler);
		return () => handlers!.delete(listener as AgentEventHandler);
	}
