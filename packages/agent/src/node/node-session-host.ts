...
> {
	...
	systemPrompt?: AgentSessionConfig<TSkill, TPromptTemplate, TTool>["systemPrompt"];
...
function isPersistedConfig(value: unknown): value is PersistedSessionConfig {
...
> implements SessionHost<TSkill, TPromptTemplate, TTool>
{
	...
	setDefaultModel(reference: ModelReference): void {
		this.options.defaultModel = structuredClone(reference);
	}

	hasActiveRunForProvider(providerId: string): boolean {
		for (const { session } of this.loaded.values()) {
			const snapshot = session.getSnapshot();
			if (snapshot.currentRunId && snapshot.model.providerId === providerId) return true;
		}
		return false;
	}
	...
	private async openUnloaded(sessionId: string): Promise<AgentSession<TSkill, TPromptTemplate, TTool>> {
		...
		try {
			...
			if (header.id !== sessionId) {
				throw new AgentRuntimeError("session", `Session directory ${sessionId} contains Session ${header.id}`);
			}
			...
			let config: PersistedSessionConfig | undefined;
			for (const entry of entries) {
				if (
					entry.type === "custom" &&
			...
			const model = this.resolveModel(modelReference);
			const tools = (await this.options.createTools?.(env)) ?? [];
			const effectiveConfig: PersistedSessionConfig = config ?? {
				providerId: model.provider,
				modelId: model.id,
	...
	private async readHeader(sessionPath: string): Promise<SessionHeader> {
		try {
			...
			if ("code" in cause && cause.code === "ENOENT") {
				throw new AgentRuntimeError("session", `Session not found: ${sessionPath}`, cause);
			}
			throw new AgentRuntimeError("session", `Failed to read Session header: ${sessionPath}`, cause);
		}
	}
}
